...
with open('mem-ratios.csv', 'w', newline='') as csvfile:
    csvwriter = csv.writer(csvfile, delimiter=',',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
    csvwriter.writerow(['Person','TestType','TestGoal', 'Right Answers', 'Answer Ratio', 'Correct Ratio', 'Recognized'])
    for x in range(5):
        for y in range(4):
            for z in range(5):
                if Mat[x][y][z][2]< 2 or (Mat[x][y][z][2]/All[x][y][z])< 0.3:
                    csvwriter.writerow([mpi[x], tti[y], z+1, Mat[x][y][z][2], YN[x][y][z]/All[x][y][z], Mat[x][y][z][2]/All[x][y][z], 0])
                else:
                    csvwriter.writerow([mpi[x], tti[y], z+1, Mat[x][y][z][2], YN[x][y][z]/All[x][y][z], Mat[x][y][z][2]/All[x][y][z], 1])